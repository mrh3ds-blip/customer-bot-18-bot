...
const TEMPLATE_CODE: string = "SERVICE_ORDER";
const TEMPLATE_TITLE = "ثبت سفارش خدمات";
const FEATURES = [
...
async function loadSettings(): Promise<Settings> {
  ...
  try {
    ...
    cachedSettings = { ...base, ...parsed, payment: { ...base.payment, ...(parsed.payment || {}) }, orders: Array.isArray((parsed as any).orders) ? (parsed as any).orders : [] };
  } catch {
...
function paymentInline() {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback("🟣 مرچنت/API درگاه", "ADM_SET_ZARINPAL")],
    [Markup.button.callback("🧪 تغییر حالت تست درگاه", "ADM_TOGGLE_ZARINPAL_SANDBOX")]
  ]);
}
...
bot.action("ADM_SET_ZARINPAL", async (ctx) => {
  ...
  await ctx.reply("مرچنت/API درگاه را بفرست.\nبرای زرین‌پال callback خودکار فعال است؛ برای سایر درگاه‌ها می‌توانی از لینک پرداخت عمومی استفاده کنی.\nاگر می‌خواهی حالت تست فعال باشد، بعد از کلید بنویس: | sandbox\n\nمثال:\nxxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx | sandbox");
});
...
bot.action(/BUY_(\d+)/, async (ctx) => {
  ...
  const rows = [];
  if (HAS_PAYMENT_GATEWAY && settings.payment.zarinpalMerchantId && item.price) rows.push([Markup.button.callback("💳 پرداخت آنلاین", "PAY_ZP_" + index)]);
...
async function handleAdminText(ctx: any, text: string) {
  ...
  if (state.action === "SET_ZARINPAL") {
    ...
    await saveSettings(settings);
    adminStates.delete(ctx.chat.id);
    await ctx.reply("مرچنت/API درگاه ذخیره شد ✅\nبرای زرین‌پال callback خودکار فعال است. برای سایر درگاه‌ها از لینک پرداخت عمومی استفاده کن.\nحالت فعلی: " + (settings.payment.zarinpalSandbox ? "تست/Sandbox" : "واقعی/Production"), adminMenu());
    return true;
  }
